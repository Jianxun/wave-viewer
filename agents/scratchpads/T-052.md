...
- Reviewer follow-up: fixed frozen path normalization so `.frozen.wave-viewer.yml` no longer gets a duplicated `.frozen` suffix, and added regression coverage.
...
- Fixed frozen export path normalization for `.frozen.wave-viewer.yml` and `.frozen.wave-viewer.yaml` to prevent doubled suffix output names.
- Added smoke regression test validating `.yml` save targets produce `*.frozen.csv` and preserve selected layout extension without renaming drift.

## PR URL
- https://github.com/Jianxun/wave-viewer/pull/61
...
## Next steps
- Await reviewer re-check on blocker resolution.
