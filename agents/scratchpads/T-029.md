...
- Addressed review blocker: cached `host/workspaceLoaded` replay now hydrates tuple payloads and legacy trace `sourceId` values before replay.
...
- `src/extension.ts`
  ...
  - Added cached workspace replay hydration on `webview/ready`:
    - Emits `host/sidePanelTraceInjected` tuple payloads for replayed traces before `host/workspaceLoaded`.
    - Backfills legacy trace `sourceId` values (`<datasetPath>::<signal>`) in replayed workspace payloads.
- `src/webview/main.ts`
...
- Tests
  - `tests/extension/smoke.test.ts`
    ...
    - Added regression test proving cached workspace replay emits tuple hydration and sourceId backfill before `host/workspaceLoaded`.
